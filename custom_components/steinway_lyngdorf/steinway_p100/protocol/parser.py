...
class ResponseParser:
    ...
    def parse_source_list(response: str) -> List["Source"]:
        ...
        Response format (newline-separated after processing):
        !SRCCOUNT(4)
        !SRC(0)"DVD player"
        ...
        if not count_match:
            raise ValueError("Invalid source list response: missing SRCCOUNT")

        expected_count = int(count_match.group(1))

        # Parse each source from remaining lines
        for line in lines[1:]:
    ...
    def parse_audio_mode_list(response: str) -> List["AudioMode"]:
        ...
        if not count_match:
            raise ValueError("Invalid audio mode list response: missing AUDMODECOUNT")

        expected_count = int(count_match.group(1))

        # Parse each mode from remaining lines
        for line in lines[1:]:
    ...
    def parse_audio_type(response: str) -> str:
        ...
        Response format: !AUDTYPE(No Information, 0.0.0) or !AUDTYPE(Dolby Atmos, 7.1.4)
        Returns: Audio type string
        """
        # Handle the parentheses format with comma-separated values
        match = re.match(r"!AUDTYPE\(([^,]+),\s*([^)]+)\)", response)
        if match:
            audio_format = match.group(1).strip()
            channels = match.group(2).strip()
            if audio_format == "No Information":
                return "No Information"
            return f"{audio_format} {channels}"

        # Fallback for quoted format if it exists
    ...
    def parse_generic_response(response: str) -> Dict[str, Any]:
        ...
        if match:
            ...
            return {"command": command, "value": value, "string": string}
